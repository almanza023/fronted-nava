...
export default {
  ...
  async asyncData({ $http }) {
    try {
      ...
      const res2 = await $http.get("perfiles");
      const perfiles = await res2.json();
      return { usuarios: usuarios.data, perfiles: perfiles.data };
    } catch (error) {
      return { error };
    }
  },
  ...
  methods: {
    ...
    async initialize() {
      try {
        let res = await this.$axios.get("/usuarios")
        this.usuarios = await res.data.data
      } catch (error) {
        console.log(error)
      }
